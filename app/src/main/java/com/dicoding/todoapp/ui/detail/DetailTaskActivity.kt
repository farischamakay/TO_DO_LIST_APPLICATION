...
class DetailTaskActivity : AppCompatActivity() {
    ...
    private lateinit var taskVM: DetailTaskViewModel
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        taskVM = ViewModelProvider(this, factory).get(DetailTaskViewModel::class.java)

        var taskId : Int?
        if (intent.hasExtra(TASK_ID)){
            taskId = intent.getIntExtra(TASK_ID, -1)
            val task = taskVM.setTaskId(taskId = taskId)
            task.observe(this, {
                if (it == null){
                    finish()
                }
